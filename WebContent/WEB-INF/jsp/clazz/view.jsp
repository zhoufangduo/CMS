...
<script type="text/javascript">
	$(function(){
		...
		<c:forEach items="${classLesson}" var="clazzLesson">
			var claLes = ${clazzLesson};
			if($("#l"+claLes) != undefined){
				$("#l"+claLes).checkbox('check');
			}
		</c:forEach>
...
<body>
	<form id="addClassForm" action="<%=request.getContextPath()%>/clazz/update" method="post">
						...
						<table style="width: 100%; height: 100%;" border="0">
							<tr>
								...
								<td width="35%">
									<div class="form-group">
										<input type="text" class="form-control" name="name" placeholder="请输入登录用户名" value="${clazz.name}" />
									</div>
								</td>
								<td width="5%"></td>
								<td width="10%" class="input_label">
								...
								<td width="35%">
									<div class="form-group">
										<select id="state" name="state" class="select-block">
											<option value="1" <c:if test="${clazz.state == '1' }">selected="selected"</c:if>>初学</option>
											<option value="2" <c:if test="${clazz.state == '2' }">selected="selected"</c:if>>开学</option>
											<option value="3" <c:if test="${clazz.state == '3' }">selected="selected"</c:if>>毕业</option>
										</select>
									</div>
								</td>
							</tr>
								...
								<td width="35%">
									<div class="form-group">
										<input type="text" class="form-control" readonly="readonly"
											...
											onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" value="${clazz.startDate}" />
									</div>
								</td>
								<td width="5%"></td>
								<td width="10%" class="input_label">
								...
								<td width="35%">
									<div class="form-group">
										<input type="text" class="form-control" readonly="readonly"
											...
											onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" value="${clazz.endDate}" />
									</div>
								</td>
							</tr>
							<tr>
										...
										<select id="teacher" name="teacher" class="select-block">
											...
											<c:forEach items="${teachers}" var="teacher">
												<option  value="${teacher.username}" 
													<c:if test="${clazz.teacher ==  teacher.username}">
														selected="selected"
													</c:if>>
							...
							<tr>
								...
								<td width="35%" colspan="4">
									<div class="form-group">
										<textarea rows="3" name="context"  cols="10" class="form-control" placeholder="请输入班级描述">${clazz.context}</textarea>
									</div>
								</td>
							</tr>
						</table>
								...
								<c:forEach items="${requestScope.lessons}" var="lesson" varStatus="status" >
									<td style="padding: 5px 10px;">
										<label class="checkbox" >
								             <input type="checkbox" name="lessons" id="l${lesson.id}"  value="${lesson.id}" data-toggle="checkbox"/>${lesson.name}
								        </label>
									</td>
