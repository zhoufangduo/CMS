...
<head>
	...
	<style type="text/css">
		...
		$(function(){
			...
			if(times.length >= 1){
				$("[name=second]").val(times[times.length-1]);
			}
			if(times.length >= 2){
				$("[name=minute]").val(times[times.length-2]);
			}
			if(times.length == 3){
				$("[name=hour]").val(times[times.length-3]);
			}
		});
	</script>
</head>
<body>
	<form id="addPaperForm" action="<%=request.getContextPath()%>/paper/update" method="post">
			...
			<table style="width: 100%; height: 100%;" border="0">
				<tr>
					...
					<td width="35%">
						<div class="form-group">
							<input type="text" class="form-control" name="name" placeholder="请输入名称" value="${paper.name}" />
						</div>
					</td>
					<td width="5%"></td>
					<td width="10%" class="input_label">
				...
				<tr>
					...
					<td width="35%" colspan="4">
						<div class="form-group">
							<textarea rows="3" cols="" class="form-control" name="introduce">${paper.introduce}</textarea>
						</div>
					</td>
				</tr>
			</table>
